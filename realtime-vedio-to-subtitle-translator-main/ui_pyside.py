...
class SubtitleUI:
    ...
    def show_login_dialog(self) -> None:
        ...
        def attempt_login():
            ...
            def worker():
                ...
                # 메인 UI 스레드 컨텍스트 명시
                QTimer.singleShot(0, dialog, finish)
        ...
        def translate():
            ...
            def worker():
                try:
                    ...
                    # 메인 UI 스레드 컨텍스트 명시
                    QTimer.singleShot(0, dialog, lambda: translation_out.setText(translated))
                except Exception as exc:
                    QTimer.singleShot(0, dialog, lambda: QMessageBox.critical(dialog, "번역 실패", str(exc)))
    ...
    def use_detected_url(self) -> None:
        ...
        if detected and not detected.startswith("감지"):
            ...
            # 메인 UI 스레드 컨텍스트 명시
            QTimer.singleShot(100, self.main_window, self.start_processing)
            ...
            try:
                ...
                if res.ok:
                    ...
                    if title and title != self.last_detected_title:
                        ...
                        # 메인 UI 스레드 컨텍스트 명시
                        QTimer.singleShot(0, self.main_window, lambda t=title: self.detected_title_label.setText(t))
                        QTimer.singleShot(0, self.main_window, lambda t=title: self.current_title_label.setText(f"제목: {t}"))
                    if url and url.startswith("http") and url != self.last_detected_url:
                        self.last_detected_url = url
                        QTimer.singleShot(0, self.main_window, lambda u=url: self.detected_url_edit.setText(u))
                        QTimer.singleShot(0, self.main_window, lambda u=url: self.current_url_label.setText(f"URL: {u}"))
            except Exception:
                pass
            time.sleep(0.5)
